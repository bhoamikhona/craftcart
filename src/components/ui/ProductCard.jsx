...
export default function ProductCard({
  ...
  return (
    <Link className="product-link block h-full" href={`/shop/${productId}`}>
      <article className="h-full flex flex-col overflow-hidden shadow-[0_24px_48px_rgba(0,0,0,0.075)] rounded-4xl max-w-[350px]">
        <div className="relative">
          <div className="product-img-container rounded-4xl overflow-hidden rounded-br-none ">
        ...
        <div className="product-info-container flex flex-col gap-3 flex-1">
          <h2 className="product-name text-xl font-bold px-8 pt-8">{name}</h2>
          <p className="text-gray-400 text-sm mb-6 px-8">{description}</p>
          ...
          <div className="flex items-center justify-between h-15 mt-auto">
            <p className="flex-1 text-center text-xl font-bold">
              ${price.toFixed(2)}
            </p>
            <button className="flex-1 flex items-center justify-center gap-2 bg-primary text-white h-full rounded-tl-4xl rounded-br-4xl cursor-pointer hover:bg-orange-600">
