...
const ProductCard = ({ product }) => {
    ...
    for (let i = 0; i < fullStars; i++) {
        stars.push(<Star key={`full-${i}`} className={`w-4 h-4 transition-colors ${FULL_STAR_CLASS}`} aria-hidden="true" />);
    }
    ...
    for (let i = 0; i < emptyStars; i++) {
        stars.push(<Star key={`empty-${i}`} className={`w-4 h-4 transition-colors ${EMPTY_STAR_CLASS}`} aria-hidden="true" />);
    }
            ...
            >
                ...
                <div className="w-full h-40 md:h-48 overflow-hidden bg-[var(--muted)] rounded-t-lg -m-4 -mt-4 mb-2 relative">
                    ...
                    {isOutOfStock && (
                        <div className="absolute inset-0 bg-black/50 flex flex-col items-center justify-center p-2">
                            ...
                            <span className="text-sm font-bold text-white uppercase tracking-wider">Sold Out</span>
                        </div>
                    )}
                </div>
                ...
                <div className="p-4 pb-2 flex flex-col justify-between h-full">
                    <div className="mb-3 flex-grow"> 
                        ...
                        <p className="text-2xl font-extrabold text-[var(--primary)]">${product.price.toFixed(2)}</p>
                    </div>
                </div>
            </Link>
            ...
            <div className="px-4 pb-4 relative z-20 -mt-2">
                 <button
                    ...
                    className={`relative w-full flex items-center justify-center text-sm font-medium shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[var(--primary)] 
                        ${isOutOfStock ? 'bg-gray-400 text-white cursor-not-allowed rounded-lg px-4 py-2' : 'btn-primary'}
                    `}
                >
                    {isOutOfStock ? <XCircle className="w-4 h-4 mr-2" /> : <ShoppingCart className="w-4 h-4 mr-2" />}
