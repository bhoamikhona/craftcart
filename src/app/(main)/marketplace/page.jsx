...
const MarketplacePage = () => {
  ...
  const getActiveFilters = () => {
    ...
    if (filters.skillLevel && filters.skillLevel !== "All")
      active.push({
        key: "skillLevel",
        label: `Skill: ${filters.skillLevel}`,
        resetValue: "All",
      });
      ...
      <div className="pt-8 md:pt-12 pb-12">
        ...
        <div className="mb-8 p-4 rounded-xl border-2 border-[var(--border-color)] bg-[var(--card-bg)] shadow-xl transition-shadow duration-300 hover:shadow-2xl">
          <div className="flex items-center">
            ...
            <svg
              className="w-6 h-6 text-[var(--primary)] opacity-70 mr-3 shrink-0"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
          ...
          <div className="md:w-3/4">
            ...
            {getActiveFilters().length > 0 && (
              <div className="mb-4 flex flex-wrap gap-2 p-3 bg-[var(--muted)] rounded-lg border border-[var(--border-color)]">
                <span className="text-sm font-semibold text-[var(--foreground)] mr-1">
                  Active Filters:
                </span>
                {getActiveFilters().map((filter) => (
                  <button
                    key={filter.key}
                    onClick={() =>
                      handleRemoveFilter(filter.key, filter.resetValue)
                    }
            ...
            {loading ? (
              <div className="flex justify-center items-center h-64">
                ...
                <span className="ml-3 text-lg text-[var(--foreground)] opacity-70">
